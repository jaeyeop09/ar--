...
let selected=null,drag=null,history=[],future=[],snap=20,gridOn=true,zoom=1,stream=null;
const names={floor:'바닥',wall:'벽',column:'기둥',roof:'지붕',window:'창문',door:'문',stairs:'계단',balcony:'발코니',solar:'태양광',tree:'나무',bench:'벤치',lamp:'조명',plant:'화분'};
...
function restore(state){canvas.innerHTML='';state.forEach(s=>addPiece(s.type,s.x,s.y,false,s.rot,s.text));selected=null;update()}
...
function addPiece(type,x,y,record=true,rot='0',text=null){
 ...
 const icon={tree:'🌳',lamp:'💡',plant:'🪴'}[type]||'';
 el.textContent=text!==null?text:icon;
 el.style.left=Math.max(0,snapTo(x))+'px';el.style.top=Math.max(0,snapTo(y))+'px';el.style.transform='rotate('+rot+'deg)';
 canvas.appendChild(el);bindPiece(el);select(el);update();return el;
}
function bindPiece(el){
 el.addEventListener('pointerdown',e=>{
  e.preventDefault();select(el);drag={el,sx:e.clientX,sy:e.clientY,ox:el.offsetLeft,oy:el.offsetTop};
 ...
 el.addEventListener('pointerup',()=>{drag=null;setStatus('2cm 단위로 배치됨');update()});
 el.addEventListener('dblclick',()=>{save();el.remove();selected=null;update();setStatus('요소 삭제됨')});
...
document.getElementById('rotate').onclick=()=>{if(selected){save();const r=(+(selected.dataset.rot||0)+90)%360;selected.dataset.rot=r;selected.style.transform='rotate('+r+'deg)';showInfo(selected);setStatus('90° 회전')}};
document.getElementById('delete').onclick=()=>{if(selected){save();selected.remove();selected=null;update();setStatus('요소 삭제됨')}};
document.addEventListener('keydown',e=>{if(e.key==='Delete')document.getElementById('delete').click();if(e.key==='r')document.getElementById('rotate').click()});
...
document.getElementById('newBtn').onclick=()=>{if(confirm('현재 설계를 모두 지울까요?')){save();canvas.innerHTML='';selected=null;update();setStatus('새 설계 시작')}};
...
// 기본 예시
[['floor',240,220],['column',260,240],['column',340,240],['column',260,340],['column',340,340],['wall',260,220],['wall',260,400],['solar',440,240],['tree',560,340]].forEach(s=>addPiece(s[0],s[1],s[2],false));
history=[];update();